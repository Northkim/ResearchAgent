...
| D1-INPUT-RECONCILE-01 | Exact input / materialization | Changing an exact binding cannot safely replace a proven prior ReAgent-managed materialization. | HIGH_CORE_CONTRACT | CONFIRMED | `D1-INPUT-02`; old 0-paper library blocked newer 1-paper library. | Materializer treats all differing target bytes as ambiguous and lacks managed atomic replacement. | Operator proves ownership, preserves old copy outside Capsule, rematerializes. | Verify prior receipt/identity, atomically replace managed copy; fail closed for user-owned/ambiguous bytes. |
| D1-INPUT-RUNNABLE-01 | Readiness | Stale or incomplete materialization can still be reported as Locally Materialized / Next: Run. | CRITICAL_CORE_CONTRACT | CONFIRMED_CROSS_WORKFLOW | `D1-INPUT-03` in Idea and again in Initial Writing. | Readiness validates an old internally consistent receipt/local file without requiring equality to current Cloud binding/current plan. | Do not run; operator compares exact binding, bytes, and receipts. | Runnable iff every current local input ID/checksum equals the current exact binding and complete plan. |
| D1-INPUT-RECEIPT-01 | Materialization receipts | Changing one sibling binding invalidates unchanged input receipts through aggregate plan checksum coupling. | HIGH_CORE_CONTRACT | CONFIRMED | `D1-INPUT-04`; unchanged Research Idea bytes became unusable after Literature change. | Per-input receipt binds the aggregate materialization-plan checksum. | Preserve/move proven managed receipt/target and regenerate full current plan. | Reconcile unchanged exact inputs independently while retaining atomic whole-plan completion. |
| D1-INPUT-REFRESH-01 | Artifact index | Newly available upstream Artifacts are not automatically reconciled before downstream materialization. | MAJOR_PRODUCT_DEFECT | CONFIRMED_CROSS_WORKFLOW | `D1-INPUT-01` at Writing→Review and Review→Revision. | Local Artifact Index refresh is a separate manual lifecycle step. | Run `artifact refresh` before materialization. | High-level continue/materialize refreshes safely and reports reconciliation. |
...
- Literal `OPEN`: **3**.
- `CONFIRMED`: **28**.
- `CONFIRMED_CROSS_WORKFLOW`: **9**.
- Unresolved evidenced (`OPEN` + both confirmed statuses): **40**.

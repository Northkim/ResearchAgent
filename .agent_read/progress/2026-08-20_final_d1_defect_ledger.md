...
| D1-WORKFLOW-ORDINAL-01 | Workflow labels | Owner-facing ordinals differ between Cloud/Local and are wrongly shared across Initial/Revision Writing roles. | MAJOR_IDENTITY_PRESENTATION | FOUND_AND_REPAIRED_POST_D1 | `D1-WORKFLOW-LABEL-01/02` preserved; R6 projection/unit/E6 tests prove explicit INITIAL/REVISION authority, unnumbered Writing roles, and deterministic exact-instance ordering for repeated non-Writing roles across Cloud and Local. | Multiple projections derived order independently and grouped by Workflow family; downstream Detail guessed role from a hard-coded version. | None needed after R6. | Preserve one exact role-aware projection; never infer Writing role from version or list position. |
| D1-WRITING-ENTRY-01 | Writing readiness UX | Input cards say Ready after Cloud selection while Local still needs materialization. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | `D1-WRITING-ENTRY-01` preserved; R6 component and controlled browser evidence renders accepted inputs as `Selected` while Local readiness remains a separate exact state. | One label conflated accepted Cloud binding with local execution readiness. | None needed after R6. | Preserve Selected for Cloud binding and reserve runnable wording for exact Local readiness. |
| D1-WRITING-UX-02 | Writing local guidance | Materialization instruction says “resume saved work” rather than preparing verified selected inputs. | MINOR_UX | FOUND_AND_REPAIRED_POST_D1 | `D1-WRITING-UX-02` preserved; R6 component/browser evidence states that materialization prepares verified local copies of selected inputs. | Generic resume copy was reused for materialization. | None needed after R6. | Keep lifecycle-specific Local handoff copy concise and exact. |
| D1-WORKSPACE-UX-01 | Workspace sync presentation | A Project manifest change is shown as every Workflow being “Local Workspace out of date.” | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | `D1-WORKSPACE-UX-01` preserved; R6 exact-Capsule acknowledgement tests and E6 show one Project-level sync notice, unchanged exact installs current, and only the new Workflow not installed. | Sync stage was projected per Workflow from Project-level manifest revision alone. | None needed after R6. | Preserve exact installed-Capsule comparison; same-instance pin mismatch remains stale. |
| D1-OVERVIEW-PRIORITY-01 | Project Overview | After final revised manuscript, dominant Current Task is still early Experiment input preparation. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | Owner screenshot preserved; R6 projection/component/E6 evidence foregrounds `Revised manuscript completed` separately from the remaining exact Workflow action. | Readiness priority hid the achieved terminal outcome behind unresolved work. | None needed after R6. | Preserve separate achieved outcome, remaining work, and next action hierarchy. |
| D1-OUTPUT-LABEL-01 | Overview / Board / Activity | Typed downstream results become generic “Workflow Output” / “Review workflow output”; latest/recent projections can look stale despite v5. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | Owner screenshots preserved; R6 shared-label tests and E6 show Initial manuscript, Review report, Revised manuscript, Experiment result, selected Idea, and paper library across surfaces. | Shared output-label registry lagged forward Artifact types. | None needed after R6. | Keep one typed Owner-label registry synchronized with published output identities. |
| D1-RETIRED-UX-01 | Workflow Board | Retired Revision remains a full peer row and top-level count can read as eight Workflows. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | Owner 7-active/1-retired evidence preserved; R6 component/browser evidence keeps active Workflows primary and retired exact history collapsed with separate counts. | Board preserved history without sufficient active/history hierarchy. | None needed after R6. | Preserve retired provenance under secondary history and separate active/historical counts. |
| D1-PROGRESS-HISTORY-01 | Progress history | A historical Idea card can say workflow completed while its round badge says in progress. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | Owner screenshot preserved; R6 Activity tests/E6 label `Progress report · round N` and `Report status` without projecting a current Workflow next action into historical cards. | Workflow-level terminal state and historical report status were rendered without scope. | None needed after R6. | Keep report/round scope explicit and current Workflow action elsewhere. |
| D1-REVIEW-PRESENTATION-01 | Review presentation | Review preview repeats large “major issue · blocking” blocks with weak issue identity and repeated limitations. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | Owner screenshot preserved; R6 v0.2 validator/component/E6 evidence shows issue ID, category, severity, one-line problem, requested revision, and status with evidence gaps collapsed. | v0.1 bounded projection foregrounded severity and repeated limitations. | Historical v0.1 remains readable; new/backfilled exact companions use v0.2. | Preserve concise primary issue identity/action and progressive disclosure. |
| D1-OUTPUTS-LAYOUT-01 | Outputs layout | Large whitespace, narrow preview column, long pages, compressed content, and wrapping COMPLETED labels reduce scanability. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | Final D1 screenshot preserved; R6 controlled Outputs screenshot shows a wider content-first responsive preview column, reduced dead space, and stable status rendering. | Fixed layout proportions did not adapt to typed preview content. | None needed after R6. | Preserve content-first responsive width and compact technical details. |
| D1-PRESENTATION-CONTINUITY-01 | Artifact presentation | Valid completed initial manuscript v4 has no Cloud preview while Review and revised manuscript do. | MAJOR_UX | FOUND_AND_REPAIRED_POST_D1 | Owner `D1-REVIEW-PREVIEW-01` preserved; R6 E5 public copied-Workspace test reports exact v4/v3 bounded companions, verifies deterministic checksums, and replays idempotently without Harness/science rerun. | Upload-only recovery skipped optional presentation reporting. | Run normal artifact refresh/backfill; exact Artifact remains final throughout. | Preserve presentation retry independent of scientific execution and never substitute presentation for evidence. |
| D1-OVERVIEW-VISUAL-01 | Project Overview | Large translucent/duplicated research-question hero text overlaps the Project header. | MAJOR_FRONTEND_VISUAL | OBSERVATION_NEEDS_CONFIRMATION | Final Owner E9 screenshot remains authoritative occurrence evidence; R6 source audit found no duplicate DOM/pseudo-element and bounded long-topic 390/1280/1440 controlled screenshots did not reproduce the overlap. | Exact environment/render root remains unproven; R6 added only safe overflow containment. | Scroll past/ignore overlay if it recurs. | Reproduce at the original Owner viewport/runtime before claiming closure; preserve a focused responsive visual regression once isolated. |

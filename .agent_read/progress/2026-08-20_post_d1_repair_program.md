...
| D1-CHECKPOINT-PRESENTATION-01 | R2 / R3 COMPLETE | Writing, Review, Revision, and Generic Experiment render bounded evidence before natural decisions. | Workflow runner bridge; Generic Experiment coordinator/detail | exact downstream tests + R3D methodology/run/result journey | 0038 forward Generic Harness publication |
| D1-APPROVAL-BRIDGE-01 | R2 / R3 COMPLETE | Natural decisions call exact runner-owned writers; Generic Experiment uses exact methodology/result decisions and the one-use browser run approval. | `workspace_cli.py`; Generic Experiment coordinator; controlled-local approval | natural decision → exact durable record; replay; R3D | 0 in R2; 0038 publication in R3 |
...
| D1-EXPERIMENT-DURABILITY-01 | R3 COMPLETE | Managed execution manifests preserve exact completed work units and local-first terminal state. | Generic Experiment workspace/coordinator | R3D interrupted work-unit checksum/attempt reuse and exactly-once terminal sync | 0038 forward publication |
| D1-EXPERIMENT-ENTRY-01 | R3 COMPLETE | Exact Idea binding/readiness now gates preparation and Run. | Experiment Detail + generic input lifecycle | R3C component + R3D browser exact selection | 0 beyond 0038 publication |
| D1-EXPERIMENT-CAPABILITY-01 | R3 COMPLETE | Reviewed paths remain accelerators; exact no-match selects the system Generic Harness without reviewed authority. | Generic coordinator/capability resolver | adapter tests + R3D generic provenance negative | 0038; 0.7/0.10 immutable |
| D1-EXPERIMENT-INGEST-01 | R3 COMPLETE | Contract-valid Generic Harness evidence admits exactly one truthful `experiment-record/v5`. | generic contracts/validator/publisher/coordinator | R3D one v5/Progress/presentation + exact Writing consumption | 0038 forward publication |
| D1-EXPERIMENT-OPERATOR-01 | R3 COMPLETE | Mutable implementation/execution/evidence uses `.reagent/experiments/<wfi>/` outside Capsule comparison. | Workspace layout, package validator, generic runtime | workspace isolation tests + R3D | 0 schema DB; additive local namespace |
| D1-EXPERIMENT-ENV-01 | R3 COMPLETE | Existing candidates are discovered; incompatibility is bounded; no install/upgrade/download occurs. | generic preflight/runtime UI | compatible/incompatible/no-silent-install tests + R3D run summary | 0 |

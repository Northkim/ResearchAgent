"""Migration and Registry identity for the first Real Writing publication."""
...
from backend.workflow_packages.production_workflows import real_writing_contract_checksum


def test_real_writing_migration_matches_runtime_authority() -> None:
    migration = import_module(
        "backend.database.migrations.versions.20260815_0024_real_writing_initial_draft"
    )
    assert migration.down_revision == "20260815_0023"
    assert migration.VERSION == "0.3.0"
    assert migration.CONTRACT_CHECKSUM == real_writing_contract_checksum()
    assert migration.CAPSULE_ID == REAL_WRITING_CAPSULE_ID
    assert migration.CAPSULE_VERSION == "0.5.0"
    assert migration.CAPSULE_CHECKSUM == REAL_WRITING_CAPSULE_CHECKSUM


def test_real_writing_registry_is_narrow_and_preserves_v1(
    ...
    with postgres_engine.connect() as connection:
        ...
        """), {"capsule_id": REAL_WRITING_CAPSULE_ID}).mappings().one()
    ...
    assert versions[0]["output_schema_id"] == "manuscript-draft/v1"
    ...
    assert [(item["requirement_key"], item["artifact_type"], item["required"])

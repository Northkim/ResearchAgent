...
| ARCH-D1 | Hybrid Project Workspace and Workflow Capsule design | **DESIGN COMPLETE; NIGHT-B6 ARTIFACT HANDOFF FOUNDATION IMPLEMENTED** — ADR 0022 defines one logical Workspace per Project and isolated versioned Capsules; B1-B5 provide persistence, desired state, Workspace/sync and Project Progress surfaces; ADR 0026/B6 adds typed immutable Artifact metadata, exact dependency binding, a local verified Index and explicit checksum-bound materialization; no production Artifact type or second Workflow is seeded |
...
than bytes. Because no production Artifact type or second Workflow is ratified,
Literature Search is not seeded with a guessed output type and all consumer
qualification remains test-only. Idea Discovery and new executable Workflows

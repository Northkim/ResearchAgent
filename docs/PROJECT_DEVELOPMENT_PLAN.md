...
| ARCH-D1 | Hybrid Project Workspace and Workflow Capsule design | **DESIGN COMPLETE; NIGHT-B7 MULTI-WORKFLOW VALIDATION IMPLEMENTED** — ADR 0022 defines one logical Workspace per Project and isolated versioned Capsules; B1-B6 provide desired state, sync, Progress and typed handoff; B7 ratifies `selected-paper-library/v1` and adds reviewed Idea Discovery as the first production consumer without cloud execution or Artifact bytes |
...
| NIGHT-B6 | Typed Artifact Reference and explicit materialization | **PASS — OWNER REVIEW REQUIRED** — additive migration `20260806_0012`, exact Progress/producer/Capsule Artifact metadata, typed consumer requirements, specific checksum-bound bindings, repository/API provenance, separate local Artifact Index, explicit safe copy and crash-recoverable receipts; cloud stores no bytes, no automatic latest, no production type/consumer or Idea Discovery |
| NIGHT-B7 | Production literature Artifact and Idea Discovery | **PASS — OWNER REVIEW REQUIRED** — additive deterministic migration `20260806_0013`, immutable Literature Search 0.4.0/Capsule 0.6.0 producer, `selected-paper-library/v1`, reviewed Idea Discovery 0.1.0 Capsule and exact `paper_library` dependency, explicit selection/materialization/run gate, Registry-driven frontend setup and end-to-end multi-Workflow validation; legacy 0.3.0/0.5.0 remains unchanged |
...
| NIGHT-F1C | Full Research Project preset and readiness UX | **PASS — OWNER ACCEPTED** — server-resolved atomic Full Research preset, five-Workflow derived readiness/next actions, same-type instance UX, exact Artifact selection and browser/CLI continuity; no pipeline persistence, automatic binding/materialization, Skill/Resource platform or new research core |
| NIGHT-F1D | Skill Registry, exact pins and Capsule delivery | **PASS — OWNER ACCEPTED** — immutable reviewed Skill Definitions/Versions, exact Workflow Version pins, Skill-backed scaffold 0.2.0 Capsules, verified local delivery/preflight and read-only visibility; no independent Skill sync, browser admin mutation, imports, marketplace, executable Skill or Resource layer |
| NIGHT-F1E | External Resource reference, binding and local resolver shell | **PASS — OWNER REVIEW READY** — Project-scoped immutable Resource metadata, exact Workflow Instance binding, separate local Resource Index, gated deterministic `LOCAL_TEST` resolver, and Experiment 0.3.0 optional requirements; GitHub/Hugging Face remain metadata-only with zero network resolution, and Experiment remains non-executing `SCAFFOLD_CORE` |
...
acknowledgement and no overall completion percentage is invented. NIGHT-B6 now
adds the bounded Artifact handoff foundation. Canonical cloud records bind
typed immutable metadata to an exact producer Instance and Progress; exact
consumer bindings never select “latest”. The local Artifact Index independently
verifies producer bytes, and explicit materialization copies through staging
into a consumer input with checksum verification and a receipt, without
changing Installed Lock or producer state. Cloud still stores metadata rather
than bytes. B7 ratifies the first production contract instead of guessing it.
Literature Search 0.4.0 publishes one self-contained, content-addressed
`selected-paper-library/v1` only after explicit successful finish. Idea
...
carry canonical `REVIEWED_CORE` or `SCAFFOLD_CORE` maturity independently from
lifecycle and review status. Idea Discovery 0.2.0/Capsule 0.2.0 remains a
reviewed local-Harness Workflow, requires an explicit user selection, and
publishes one immutable content-addressed `selected-research-idea/v1` through
the existing Progress-to-Artifact path. Idea 0.1.0 and every Literature pin
remain unchanged, and existing instances never auto-upgrade. Code-level

          ...
          {artifacts.data.artifacts.map((artifact) => {
            ...
            return (
              <article className="output-work-row" key={artifact.artifact_id}>
                <div className="output-work-summary"><div><p className="eyebrow">Output</p><h2>{outputLabel(artifact.artifact_type, artifact.artifact_schema_version)}</h2><p>Produced by <Link href={`/projects/${projectId}/workflows/${artifact.producer_workflow_instance_id}`}>{producer?.friendly_instance_label ?? producer?.workflow_display_name ?? "Project Workflow"}</Link>.</p></div>
                <dl><div><dt>Outcome</dt><dd>{producer?.research_status.replaceAll("_", " ") ?? artifact.state.replaceAll("_", " ")}</dd></div><div><dt>Produced</dt><dd>{formatDateTime(artifact.produced_at)}</dd></div><div><dt>Round</dt><dd>{artifact.producer_execution_round}</dd></div>{report?.normalized_record?.warnings.length ? <div className="output-limitation"><dt>Limitation</dt><dd>{report.normalized_record.warnings[0]}</dd></div> : null}</dl></div>
                <div className="output-work-preview">{artifact.artifact_schema_version === "experiment-record/v4" || artifact.artifact_schema_version === "experiment-record/v5" ? <ExperimentPresentationView artifact={artifact} /> : null}
                {["selected-paper-library/v1", "selected-research-idea/v1", "manuscript-draft/v4", "review-report/v3", "manuscript-draft/v5"].includes(artifact.artifact_schema_version) ? <ArtifactPresentationPreview artifact={artifact} /> : null}</div>

...
function GenericExperimentDetail({
  ...
  const taskTitle = finalizedResult
  ...
  const taskExplanation = finalizedResult
    ? "The final scientific result and its reported limitations are available below."
    : approvalChanged || approvalStatus === "SUPERSEDED"
  ...
  const currentTask = <section className="experiment-current-task" data-attention-state={approvalStatus === "APPROVED" || checkpoint === "PREPARATION_COMPLETE" || finalizedResult ? "NORMAL" : "OWNER_ACTION_REQUIRED"} aria-labelledby="experiment-current-task-title"><p className="eyebrow">Current task</p><h2 id="experiment-current-task-title">{taskTitle}</h2><p>{taskExplanation}</p>{checkpointDetail && checkpointDetail !== taskExplanation && !approvalChanged && !finalizedResult ? <p className="muted-copy">{checkpointDetail}</p> : null}{approvalStatus === "REQUESTED" && !approvalChanged ? <p>This approval applies to one exact prepared experiment and one local execution attempt. The browser records authorization; it does not start the local process.</p> : null}{approvalChanged ? <p role="alert" className="attention-copy">The prepared experiment changed after approval. Review the updated experiment before continuing.</p> : null}<div className="experiment-task-actions">
    ...
    {approvalStatus === "APPROVED" && !finalizedResult ? <a className="button button-primary" href="#local-workflow">Continue in Local Workspace</a> : null}
  ...
  return <div className="page-stack workflow-detail-page">
    ...
    <section className="plain-section" aria-labelledby="research-objective-title"><p className="eyebrow">Research objective</p><h2 id="research-objective-title">{objective}</h2><p>{objective}</p><p className="muted-copy">{ideaBinding ? "Source · Selected research idea · exact version recorded" : "Select the exact Research Idea before experiment preparation."}</p>{ideaProducer ? <Link className="text-link" href={`/projects/${project.project_id}/workflows/${ideaProducer.workflow_instance_id}`}>View research idea →</Link> : null}
      ...
      {state.action.next_action.code === "SELECT_INPUT" ? <WorkflowInputSetup projectId={project.project_id} instance={instance} instances={instances} projections={progress.instances} requirements={requirements} dependencies={dependencies} command={state.action.next_action.command} code={state.action.next_action.code} /> : ideaBinding ? <details className="secondary-control"><summary>Change research idea</summary><WorkflowInputSetup projectId={project.project_id} instance={instance} instances={instances} projections={progress.instances} requirements={requirements} dependencies={dependencies} command={state.action.next_action.command} code={state.action.next_action.code} /></details> : null}
    </section>
    ...
    {ideaBinding && experimentPathSelected ? <>
      ...
      {runCheckpoint && !finalizedResult ? <section className="plain-section" aria-labelledby="exact-run-title"><p className="eyebrow">Ready to run</p><h2 id="exact-run-title" ref={exactRunHeadingRef} tabIndex={-1}>Exact run summary</h2>{runSummary ? <div className="input-readiness-list">
...
function DownstreamWorkflowDetail({
  ...
  return <div className="page-stack workflow-detail-page">
    ...
    <section id="inputs" className="plain-section" aria-labelledby="downstream-sources-title"><p className="eyebrow">Source</p><h2 id="downstream-sources-title">{sourceTitle}</h2>
      ...
      {state.action.next_action.code === "SELECT_INPUT" ? <WorkflowInputSetup projectId={project.project_id} instance={instance} instances={instances} projections={progress.instances} requirements={requirements} dependencies={dependencies} command={state.action.next_action.command} code={state.action.next_action.code} /> : state.action.next_action.code === "MATERIALIZE" ? <details className="secondary-control"><summary>Add or change evidence</summary><WorkflowInputSetup projectId={project.project_id} instance={instance} instances={instances} projections={progress.instances} requirements={requirements} dependencies={dependencies} command={state.action.next_action.command} code={state.action.next_action.code} /></details> : null}
    </section>
      ...
      <div className="workflow-support-grid">
        <section id="inputs" className="plain-section" aria-labelledby="workflow-inputs-title">
          ...
          {requirements.length && state.action.next_action.code === "SELECT_INPUT" ? (
            <WorkflowInputSetup projectId={projectId} instance={instance} instances={instances.data.items} projections={progress.data.instances} requirements={requirements} dependencies={dependencies} command={state.action.next_action.command} code={state.action.next_action.code} />
          ) : dependencies.length ? (
            <details className="secondary-control"><summary>Manage input bindings</summary><WorkflowInputSetup projectId={projectId} instance={instance} instances={instances.data.items} projections={progress.data.instances} requirements={requirements} dependencies={dependencies} command={state.action.next_action.command} code={state.action.next_action.code} /></details>
          ) : null}
        </section>

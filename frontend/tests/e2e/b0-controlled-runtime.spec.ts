...
type FixtureManifest = {
  run_id: string; project_id: string; project_name: string; instances: Record<string, string>;
};
const viewports = [
  { width: 1440, height: 900 }, { width: 1280, height: 800 }, { width: 390, height: 844 },
] as const;
...
test("qualifies the controlled browser surface at all B0 viewports", async ({ page, request }) => {
  ...
  const screenshotRoot = process.env.REAGENT_B0_SCREENSHOT_DIR!;
  const launchMarker = process.env.REAGENT_B0_BROWSER_LAUNCH_MARKER!;
  ...
  page.on("request", (requestEvent) => {
    ...
    const permitted = ["http:", "https:", "ws:", "wss:"].includes(url.protocol)
      && loopbackHosts.has(url.hostname) && allowedPorts.has(url.port);
    if (!permitted) {
  ...
  const assertBrowserSafe = async () => {
    ...
    expect(pageErrors, "unexpected pageerror").toEqual([]);
    expect(externalRequests, "unexpected non-loopback browser request").toEqual([]);
  };
  ...
  expect(state("literature-search-local-experimental")).toMatchObject({ research_status: "COMPLETED", result_count: 1 });
  expect(state("idea-discovery-local-experimental")).toMatchObject({ research_status: "BLOCKED" });
  expect(state("writing-local-experimental")).toMatchObject({ research_status: "BLOCKED" });
  ...
  for (const viewport of viewports) {
    ...
    await expect(card("idea-discovery-local-experimental").getByText(/Blocked until an exact controlled input/)).toBeVisible();
    await expect(card("writing-local-experimental").getByText(/Awaiting owner action/)).toBeVisible();
    ...
    await page.screenshot({ path: join(screenshotRoot,
      `projects-workflows__${viewport.width}x${viewport.height}__controlled-states__fold.png`), fullPage: false });
    await assertBrowserSafe();
  }
  await assertBrowserSafe();
});
